...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO customers (customername, customercontact, customeraddress, customerpostalcode, customerstate) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssis", $customerName, $customerContact, $customerAddress, $customerPostalCode, $customerState);

        // Execute the statement and check for errors
        if ($stmt->execute()) {
            echo "<script>alert('New customer created successfully'); window.location.href='viewcustomer.php';</script>";
        } else {
